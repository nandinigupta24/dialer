				...
				if (isset($match[ 'operator' ]))
				{
					...
					elseif ($operator === '<>' || $operator === '><')
					{
						if ($type === 'array')
						{
							...
							$data_type = (isset($value[ 0 ]) && is_numeric($value[ 0 ]) && is_numeric($value[ 1 ])) ? PDO::PARAM_INT : PDO::PARAM_STR;

							$map[ $map_key . 'a' ] = [(isset($value[ 0 ]) ? $value[ 0 ]:''), $data_type];
							$map[ $map_key . 'b' ] = [(isset($value[ 1 ]) ? $value[ 1 ] : ''), $data_type];
						}
					}

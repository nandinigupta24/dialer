...
#  - $user
#  - $pass
#  - $function - ('customer-create','update_lead','version','sounds_list','moh_list','vm_list','blind_monitor','agent_ingroup_info','add_list',etc...)
#  - $source - ('vtiger','webform','adminweb')
#  - $format - ('text','debug')
# optional callback variables for customer-create/update_lead
#  - $callback -	('Y,'N','REMOVE')
...
# 91216-0331 - Added duplication check features to customer-create function
# 100118-0543 - Added new Australian and New Zealand DST schemes (FSO-FSA and LSS-FSA)
# 100704-1148 - Added custom fields inserts to the customer-create function
...
# 110705-1928 - Added options for USACAN 4th digit prefix(no 0 or 1) and valid areacode filtering to customer-create
# 110821-2318 - Added update_phone, add_phone_alias, update_phone_alias functions
# 110928-2110 - Added callback options to customer-create and update_lead
...
# 121116-1938 - Added state call time restrictions to customer-create hopper insert function
...
# 150123-1611 - Fixed issue with list local call times and customer-create
...
# 161020-1042 - Added lookup_state option to customer-create, added 10-digit validation if usacan_areacode_check enabled
...
require("../admin_dial/dbconnect_mysqli.php");
require("../admin_dial/functions.php");
...
if (isset($_GET["main_phone"]))				{$phone_number=$_GET["main_phone"];}
	elseif (isset($_POST["main_phone"]))		{$phone_number=$_POST["main_phone"];}
if (isset($_GET["leadcode"]))			{$vendor_lead_code=$_GET["leadcode"];}
	elseif (isset($_POST["leadcode"]))	{$vendor_lead_code=$_POST["leadcode"];}
...
if (isset($_GET["queue"]))				{$add_to_hopper=$_GET["queue"];}
	elseif (isset($_POST["queue"]))		{$add_to_hopper=$_POST["queue"];}
...
if ($non_latin < 1)
	...
	$token=preg_replace('/[^-_0-9a-zA-Z]/','',$token);
...
if($token){
  $userDetails = user_detail_by_token($token);

  if(count($userDetails)>0){
      $user = $userDetails['user'];
      $pass = $userDetails['pass'];
  }else {
    $auth_message = 'UnAuthorised';
    $VDdisplayMESSAGE = "ERROR: Your token is not allowed: $ip";
		Header ("Content-type: text/html; charset=utf-8");
		echo "$VDdisplayMESSAGE: |$token|$auth_message|\n";
		exit;
  }
}
...
if ( ($api_list_restrict > 0) and ( ($function == 'customer-create') or ($function == 'update_lead') or ($function == 'update_list') or ($function == 'list_info') or ($function == 'list_custom_fields') ) )
	{
...
if (file_exists('options.php'))
	{
	require('../admin_dial/options.php');
	}
...
################################################################################
### customer-create - inserts a lead into the vicidial_list table
################################################################################
if ($function == 'customer-create')
	{
	if(strlen($source)<2)
		{
		$result = 'ERROR';
		...
		if ($modify_leads < 1)
			{
			$result = 'ERROR';
			$result_reason = "customer-create USER DOES NOT HAVE PERMISSION TO ADD LEADS TO THE SYSTEM";
			...
			if ($api_list_restrict > 0)
				...
				if (!preg_match("/ $list_id /",$allowed_lists))
					{
					$result = 'ERROR';
					$result_reason = "customer-create NOT AN ALLOWED LIST ID";
			...
			if ( ($nanpa_ac_prefix_check == 'Y') or (preg_match("/NANPA/i",$tz_method)) )
				...
				if ($vicidial_nanpa_prefix_codes_count < 10)
					...
					$result = 'NOTICE';
					$result_reason = "customer-create NANPA options disabled, NANPA prefix data not loaded";
			...
			if ( (strlen($phone_number)<6) || (strlen($phone_number)>16) )
				{
				$valid_number=0;
				$result_reason = "customer-create INVALID PHONE NUMBER LENGTH";
				}
			if ( ($usacan_prefix_check=='Y') and ($valid_number > 0) )
				{
				$USprefix = 	substr($phone_number, 3, 1);
				if ($DB>0) {echo "DEBUG: customer-create prefix check - $USprefix|$phone_number\n";}
				if ($USprefix < 2)
					{
					$valid_number=0;
					$result_reason = "customer-create INVALID PHONE NUMBER PREFIX";
					}
				}
			if ( ($usacan_areacode_check=='Y') and ($valid_number > 0) )
				...
				if ($DB>0) {echo "DEBUG: customer-create areacode check query - $stmt\n";}
				$rslt=mysql_to_mysqli($stmt, $link);
				$row=mysqli_fetch_row($rslt);
				$valid_number=$row[0];
				if ( ($valid_number < 1) || (strlen($phone_number)>10) || (strlen($phone_number)<10) )
					{
					$result_reason = "customer-create INVALID PHONE NUMBER AREACODE";
					}
				}
			if ( ($nanpa_ac_prefix_check=='Y') and ($valid_number > 0) )
				...
				if ($valid_number < 1)
					{
					$result_reason = "customer-create INVALID PHONE NUMBER NANPA AREACODE PREFIX";
					}
				}
			if ($valid_number < 1)
				{
				...
				if ( ($dnc_check == 'Y') or ($dnc_check == 'AREACODE') )
					...
					if ($dnc_check == 'AREACODE')
						...
						{$stmt="SELECT count(*) from vicidial_dnc where phone_number='$phone_number';";}
					if ($DB>0) {echo "DEBUG: customer-create query - $stmt\n";}
					$rslt=mysql_to_mysqli($stmt, $link);
					$row=mysqli_fetch_row($rslt);
					$dnc_found=$row[0];
					...
					if ($dnc_found > 0)
						{
						$result = 'ERROR';
						$result_reason = "customer-create PHONE NUMBER IN DNC";
						echo "$result: $result_reason - $phone_number|$user\n";
						$data = "$phone_number";
						api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
						exit;
				...
				if ( ($campaign_dnc_check == 'Y') or ($campaign_dnc_check == 'AREACODE') )
					...
					if ($dnc_found > 0)
						{
						$result = 'ERROR';
						$result_reason = "customer-create PHONE NUMBER IN CAMPAIGN DNC";
				...
				if (preg_match("/DUPLIST/i",$duplicate_check)) # duplicate check within list
					...
					if ($duplicate_found > 0)
						{
						$result = 'ERROR';
						$result_reason = "customer-create DUPLICATE PHONE NUMBER IN LIST";
				...
				if (preg_match("/DUPCAMP/i",$duplicate_check)) # duplicate check within campaign lists
					...
					if ($duplicate_found > 0)
						{
						$result = 'ERROR';
						$result_reason = "customer-create DUPLICATE PHONE NUMBER IN CAMPAIGN LISTS";
				...
				if (preg_match("/DUPSYS/i",$duplicate_check)) # duplicate check within entire system
					...
					if ($duplicate_found > 0)
						{
						$result = 'ERROR';
						$result_reason = "customer-create DUPLICATE PHONE NUMBER IN SYSTEM";
						$data = "$phone_number|$list_id|$duplicate_lead_id|$duplicate_lead_list";
						echo "$result: $result_reason - $data\n";
						api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
						exit;
				...
				if (preg_match("/DUPTITLEALTPHONELIST/i",$duplicate_check)) # duplicate title/alt_phone check within list
					...
					if ($duplicate_found > 0)
						{
						$result = 'ERROR';
						$result_reason = "customer-create DUPLICATE TITLE ALT_PHONE IN LIST";
				...
				if (preg_match("/DUPTITLEALTPHONECAMP/i",$duplicate_check)) # duplicate title/alt_phone check within campaign lists
					...
					if ($duplicate_found > 0)
						{
						$result = 'ERROR';
						$result_reason = "customer-create DUPLICATE TITLE ALT_PHONE IN CAMPAIGN LISTS";
				...
				if (preg_match("/DUPTITLEALTPHONESYS/i",$duplicate_check)) # duplicate title/alt_phone check within entire system
					...
					if ($duplicate_found > 0)
						{
						$result = 'ERROR';
						$result_reason = "customer-create DUPLICATE TITLE ALT_PHONE IN SYSTEM";
						$data = "$title|$alt_phone|$list_id|$duplicate_lead_id|$duplicate_lead_list";
						echo "$result: $result_reason - $data\n";
						api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
						exit;
				...
				if (preg_match("/DUPNAMEPHONELIST/i",$duplicate_check)) # duplicate name/phone check within list
					...
					if ($duplicate_found > 0)
						{
						$result = 'ERROR';
						$result_reason = "customer-create DUPLICATE NAME PHONE IN LIST";
				...
				if (preg_match("/DUPNAMEPHONECAMP/i",$duplicate_check)) # duplicate name/phone check within campaign lists
					...
					if ($duplicate_found > 0)
						{
						$result = 'ERROR';
						$result_reason = "customer-create DUPLICATE NAME PHONE IN CAMPAIGN LISTS";
				...
				if (preg_match("/DUPNAMEPHONESYS/i",$duplicate_check)) # duplicate name/phone check within entire system
					...
					if ($duplicate_found > 0)
						{
						$result = 'ERROR';
						$result_reason = "customer-create DUPLICATE NAME PHONE IN SYSTEM";
						$data = "$first_name|$last_name|$phone_number|$list_id|$duplicate_lead_id|$duplicate_lead_list";
						echo "$result: $result_reason - $data\n";
						api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
						exit;
				...
				if ($affected_rows > 0)
					...
					$result = 'SUCCESS';
					$result_reason = "customer-create LEAD HAS BEEN ADDED";
					...
					if (strlen($multi_alt_phones) > 5)
						...
						$map_count = count($map);
						if ($DB>0) {echo "DEBUG: customer-create multi-al-entry - $a|$map_count|$multi_alt_phones\n";}
						$g++;
						...
						$result = 'NOTICE';
						$result_reason = "customer-create MULTI-ALT-PHONE NUMBERS LOADED";
					...
					if ($custom_fields == 'Y')
						...
						if ($custom_fields_enabled > 0)
							...
							if ($tablecount_to_print > 0)
								...
								if (strlen($CFinsert_SQL)>3)
									...
									if ($custom_insert_count > 0)
										...
										$result = 'NOTICE';
										$result_reason = "customer-create CUSTOM FIELDS VALUES ADDED";
										...
										{
										$result = 'NOTICE';
										$result_reason = "customer-create CUSTOM FIELDS NOT ADDED, NO FIELDS TO UPDATE DEFINED";
									...
									{
									$result = 'NOTICE';
									$result_reason = "customer-create CUSTOM FIELDS NOT ADDED, NO FIELDS DEFINED";
									echo "$result: $result_reason - $phone_number|$lead_id|$list_id\n";
									$data = "$phone_number|$lead_id|$list_id";
									api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
									}
								...
								{
								$result = 'NOTICE';
								$result_reason = "customer-create CUSTOM FIELDS NOT ADDED, NO CUSTOM FIELDS DEFINED FOR THIS LIST";
								echo "$result: $result_reason - $phone_number|$lead_id|$list_id\n";
								$data = "$phone_number|$lead_id|$list_id";
								api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
								}
							...
							{
							$result = 'NOTICE';
							$result_reason = "customer-create CUSTOM FIELDS NOT ADDED, CUSTOM FIELDS DISABLED";
					...
					if ($add_to_hopper == 'Y')
						...
						if ($dialable < 1)
							{
							$result = 'NOTICE';
							$result_reason = "customer-create NOT ADDED TO HOPPER, OUTSIDE OF LOCAL TIME";
							...
							if ($Haffected_rows > 0)
								...
								$result = 'NOTICE';
								$result_reason = "customer-create ADDED TO HOPPER";
								...
								{
								$result = 'NOTICE';
								$result_reason = "customer-create NOT ADDED TO HOPPER";
						...
						if ($camp_count > 0)
							...
							if ($callback_type == 'USERONLY')
								...
								if ($user_recs > 0)
									...
									{
									$result = 'NOTICE';
									$result_reason = "customer-create SCHEDULED CALLBACK NOT ADDED, USER NOT VALID";
							...
							if ($valid_callback > 0)
								...
								$result = 'NOTICE';
								$result_reason = "customer-create SCHEDULED CALLBACK ADDED";
							...
							{
							$result = 'NOTICE';
							$result_reason = "customer-create SCHEDULED CALLBACK NOT ADDED, CAMPAIGN NOT VALID";
					...
					{
					$result = 'ERROR';
					$result_reason = "customer-create LEAD HAS NOT BEEN ADDED";
...
function api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data)
    ...
    $function = $funciton=='customer-create'?'customer-create' : $funciton;
		$stmt="INSERT INTO vicidial_api_log set user='$user',agent_user='$agent_user',function='$function',value='$value',result=\"$result\",result_reason='$result_reason',source='$source',data=\"$data\",api_date='$NOW_TIME',api_script='$api_script',run_time='$TOTALrun',webserver='$webserver_id',api_url='$url_id',remote_ip='$ip';";
		$rslt=mysql_to_mysqli($stmt, $link);
...
// Get User and password by token
function user_detail_by_token($token)
{
  global $link;
  $ip = getenv("REMOTE_ADDR");
  $today = date('Y-m-d');
  $stmt = "SELECT user, pass FROM vicidial_users where api_token='$token' and FIND_IN_SET('$ip', api_ip_list) AND '$today' between api_start_date and api_end_date;";
  $rslt=mysql_to_mysqli($stmt, $link);

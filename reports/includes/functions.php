...
$folder = 'reports';
$mainFolder = 'utgone';
$myWorkingDir = strtolower('/'.$folder.'/');
...
function base_url($string = '', $option = 0) {
    global $myWorkingDir, $publicAssestUrl, $folder , $mainFolder;
    return $publicAssestUrl.($option ? $mainFolder : $folder).'/'.$string;
}

function redirect($string = '', $option = 0) {
    header('location:'. base_url($string, $option));
    exit;
}
...
function get_validation($type, $tableSyntax,$operator,$database=NULL) {
    $OperatorAssign = '';
    if($operator != NULL){
   ...
   $dataGroup = $_SESSION['CurrentLogin']['CampaignID'];
    if($UserGroup == 'ADMIN'){
        $query = '';
    }else{
        switch ($type) {
            case 'agent':
               $query = $OperatorAssign.$tableSyntax . ".user_group IN ('" . implode("','",$_SESSION['CurrentLogin']['allowed_teams_access']) . "')";
                break;
            case 'campaign':
                $query = $OperatorAssign.$tableSyntax . ".campaign_id IN ('" . implode("','",$dataGroup) . "')";
                break;
            case 'list':
                $query = $OperatorAssign.$tableSyntax . ".campaign_id IN ('" . implode("','",$dataGroup) . "')";
                break;
...
/*Login User Setup*/
function login_user_setup($database,$user){
    $UserDetail = $database->get('vicidial_users',['user','full_name','user_level','user_group','role_id','user_id'],['user'=>$user]);

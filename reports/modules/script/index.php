...
$data = $DBScripts->select('customer', '*', ["AND" => ['created_at[>=]' => $start, 'created_at[<=]' => $end, 'question_1[!]' => ''], 'ORDER' => ['id' => 'DESC']]);
//$data = $DBScripts->select('customer', '*', ["AND" => ['created_at[>=]' => $start, 'created_at[<=]' => $end], 'ORDER' => ['id' => 'DESC']]);
?>        
                ...
                <div class="box">
                    <div class="with-border">
                        <div class="panel-heading">
                            <div class="panel-title"> <span class="fa fa-list-alt"></span>Script Report</div>
                            ...
                            <table id="table-list-campaigns" class="table table-bordered table-striped" style="width:100%">
                                <thead class="bg-success">
                                    <tr>
                                        ...
                                        <th>Refused LI</th>
                                        <th>Smoker catagory</th>
                                        <th>DM</th>
                                        <th>LI Quote</th>
                                        <th>Current Life Policy</th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    foreach ($data as $recording) {
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo $recording['question_7']; ?></td>
                                            <td><?php echo $recording['question_8']; ?></td>
                                            <td><?php echo $recording['question_9']; ?></td>
                                            <td><?php echo $recording['interested']; ?></td>
                                            <td><?php echo $recording['final']; ?></td>
...
<script>
    $(function () {
        ...
        $('#daterange-btn').daterangepicker(
                ...
                function (start, end) {
                    ...
                    window.location.href = '<?php echo base_url('script') ?>?start=' + start + '&end=' + end;
                }
        );
    })

    var table = $('#table-list-campaigns').DataTable({dom: 'Bfrtip',
        buttons: [
    ...
    $(document).on('click','.Remove',function(){
        ...
        swal({
                    ...
                    if (isConfirm) {
                        $.post('<?php echo base_url('script/ajax') ?>?action=deleteById', {ID:ID}, function (resp) {
    ...
    $(document).on('click','.RemoveBulk',function(){
        swal({
                    ...
                    if (isConfirm) {
                        $.post('<?php echo base_url('script/ajax') ?>?action=RemoveBulk&type=SEOBIRD', {start:'<?php echo $start;?>',end:'<?php echo $end;?>'}, function (resp) {
                            swal("Deleted!", "Your data been deleted.", "success");
    //                        table.ajax.reload();
                            window.location.reload();
                        });

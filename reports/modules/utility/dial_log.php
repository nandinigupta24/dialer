                ...
                <div class="box">
                    <div class="with-border">
                        <div class="panel-heading">
                            <div class="panel-title"> <span class="fa fa-calculator"></span>Dial Log Report</div>
...
//                        }
                        ...
                        } elseif (isset($_POST["hangup_cause"])) {
                            $hangup_cause = $_POST["hangup_cause"];
                        }
                        if (isset($_GET["sip_hangup_cause"])) {
                            $sip_hangup_cause = $_GET["sip_hangup_cause"];
                        } elseif (isset($_POST["sip_hangup_cause"])) {
...
#### SIP response code directory
                        ...
                        if (!isset($sip_hangup_cause)) {
                            $sip_hangup_cause = array();
                        }
...
##### END log visit to the vicidial_report_log table #####
                        ...
                        if (strlen($sip_hangup_cause_SQL) > 0) {
                            $sip_hangup_cause_SQL = "and sip_hangup_cause in ($sip_hangup_cause_SQL)";
                        }
...
//                        $TEXT.="<PRE><font size=2>\n";
                        ...
                        if ($SUBMIT && $query_date) {
                            $stmt = "SELECT * From vicidial_dial_log where call_date>='$start $query_date_D' and call_date<='$end $query_date_T' $server_ip_SQL $sip_hangup_cause_SQL order by call_date asc";
                            $rslt = mysql_to_mysqli($stmt, $link);
                            
                            if (!$lower_limit) {
                                $lower_limit = 1;
                            }
                            if ($lower_limit + 999 >= mysqli_num_rows($rslt)) {
                                $upper_limit = ($lower_limit + mysqli_num_rows($rslt) % 1000) - 1;
                            } else {
...
//                            $TEXT.="--- " . _QXZ("DIAL LOG RECORDS FOR") . " $query_date, $query_date_D " . _QXZ("TO") . " $query_date_T $server_rpt_string, $HC_rpt_string\n --- " . _QXZ("RECORDS") . " #$lower_limit-$upper_limit               <a href=\"$PHP_SELF?SUBMIT=$SUBMIT&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS$sip_hangup_causeQS&lower_limit=$lower_limit&upper_limit=$upper_limit&file_download=1\">[" . _QXZ("DOWNLOAD") . "]</a>\n";
                            ...
                            $HTML.="<th colspan='12'><font size='2'>" . _QXZ("DIAL LOG RECORDS FOR") . " $start $query_date_D " . _QXZ("TO") . " $end $query_date_T $server_rpt_string, $HC_rpt_string, " . _QXZ("RECORDS") . " #$lower_limit-$upper_limit</font></th>";
//                            $HTML.="<th><font size='2'><a href=\"$PHP_SELF?SUBMIT=$SUBMIT&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS$sip_hangup_causeQS&lower_limit=$lower_limit&upper_limit=$upper_limit&file_download=1\">[" . _QXZ("DOWNLOAD") . "]</a></font></th>";
                            ...
                            if (mysqli_num_rows($rslt) > 0) {
                                ...
                                while ($row = mysqli_fetch_array($rslt)) {
                                    $i++;
                                    if (strlen($row["extension"]) > 20) {
                                        $row["extension"] = substr($row["extension"], 0, 17) . "...";
                                    }
                                    if (strlen($row["caller_code"]) > 20) {
                                        $row["caller_code"] = substr($row["caller_code"], 0, 17) . "...";
                                    }
                                    if (strlen($row["context"]) > 20) {
                                        $row["context"] = substr($row["context"], 0, 17) . "...";
                                    }
                                    if (strlen($row["outbound_cid"]) > 40) {
                                        $row["outbound_cid"] = substr($row["outbound_cid"], 0, 37) . "...";
                                    }
                                    if ($i >= $lower_limit && $i <= $upper_limit) {
                                        ...
                                        $HTML.="<tr>";
                                        $HTML.="<td><font size='2'>" . $row["caller_code"] . "</font></td>";
                                        ...
                                        $HTML.="<td><font size='2'>" . $row["extension"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["channel"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["context"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["timeout"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["outbound_cid"] . "</font></td>";
                            ...
                            if (($lower_limit + 1000) < mysqli_num_rows($rslt)) {
                                if ($upper_limit + 1000 >= mysqli_num_rows($rslt)) {
                                    $max_limit = mysqli_num_rows($rslt) - $upper_limit;
                                } else {
                                    $max_limit = 1000;
                                }
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label for="example_input_full_name">SIP Response</label>
                    <select name="sip_hangup_cause[]" multiple="" class="form-control select2" style="width:100%;">
                        <?php
                        if (preg_match('/--ALL--/', $sip_hangup_causes_string)) {
                            echo "<option value=\"--ALL--\" selected>-- " . _QXZ("ALL SIP CAUSES") . " --</option>\n";
                        } else {
                            echo "<option value=\"--ALL--\">-- " . _QXZ("ALL SIP CAUSES") . " --</option>\n";
                        }

                        $o = 0;
                        while ($sip_responses_to_print > $o) {
                            if (preg_match("/\|$master_sip_response_directory[$o]\|/", $sip_hangup_causes_string)) {
                                echo "<option selected value=\"$master_sip_response_directory[$o]\">$master_sip_response_directory[$o] - $master_sip_response_verbiage_directory[$o]</option>\n";
                            } else {
                                echo "<option value=\"$master_sip_response_directory[$o]\">$master_sip_response_directory[$o] - $master_sip_response_verbiage_directory[$o]</option>\n";
                            }
                            $o++;
                        }
                        ?>

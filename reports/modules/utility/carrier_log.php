                ...
                <div class="box">
                    <div class="with-border">
                        <div class="panel-heading">
                            <div class="panel-title"> <span class="fa fa-building"></span>Carrier Log Report</div>
                            <ul class="nav panel-tabs">
                                <li><a href="javascript:void(0);" data-toggle="modal" data-target="#modal-right"><i class="fa fa-search" title="Search"></i></a></li>
...
//                        $MAIN.="</TD></TR></TABLE>\n";
                        if ($SUBMIT && $server_ip_ct > 0) {
                            $stmt = "select hangup_cause, dialstatus, count(*) as ct From vicidial_carrier_log where call_date>='$start $query_date_D' and call_date<='$end $query_date_T' $server_ip_SQL group by hangup_cause, dialstatus order by hangup_cause, dialstatus";
                            ...
                            if (mysqli_num_rows($rslt) > 0) {
                                ...
                                $HTML.="<table class='table table-bordered pull-left' style='width:45%;'>";
                                $HTML.="<thead class='bg-warning'>";
                                $HTML.="<tr>";
//                                $HTML.="<th colspan='3'>" . _QXZ("DIAL STATUS BREAKDOWN FOR") . " $query_date, $query_date_D " . _QXZ("TO") . " $query_date_T $server_rpt_string</th>";
                                $HTML.="<th colspan='3'><font size='2'>DIAL STATUS BREAKDOWN</font></th>";
                                $HTML.="</tr>";
                                $HTML.="<tr>";
                                ...
                                $stmt = "select sip_hangup_cause,sip_hangup_reason,count(*) as ct From vicidial_carrier_log where call_date>='$start $query_date_D' and call_date<='$end $query_date_T' $server_ip_SQL group by sip_hangup_cause,sip_hangup_reason order by sip_hangup_cause,sip_hangup_reason";
                                $rslt = mysql_to_mysqli($stmt, $link);
                                $TEXT.="<PRE><font size=2>\n";
                                if ($DB) {
                                    $TEXT.=$stmt . "\n";
                                ...
                                if (mysqli_num_rows($rslt) > 0) {
                                    ...
                                    $HTML.="<table class='table table-bordered pull-right' style='width:45%;'>";
                                    $HTML.="<thead class='bg-primary'>";
                                    $HTML.="<tr>";
//                                    $HTML.="<th colspan='3'>" . _QXZ("SIP ERROR REASON BREAKDOWN FOR") . " $query_date, $query_date_D " . _QXZ("TO") . " $query_date_T $server_rpt_string</th>";
                                    $HTML.="<th colspan='3'><font size='2'>SIP ERROR REASON BREAKDOWN</font></th>";
                                    $HTML.="</tr>";
                                    $HTML.="<tr>";
                                    ...
                                    while ($row = mysqli_fetch_array($rslt)) {
                                        ...
                                        $TEXT.="|\n";
                                        $HTML.="<tr>";
                                        $HTML.="<td><font size='2'>" . $row["sip_hangup_cause"] . "</font></th>";
                                        $HTML.="<td><font size='2'>" . $row["sip_hangup_reason"] . "</font></th>";
                                        $HTML.="<td><font size='2'>" . $row["ct"] . "</font></th>";
                                        $HTML.="</tr>\n";
                                        $total_count+=$row["ct"];
                                    }
                                    $TEXT.="+----------+--------------------------------+---------+\n";
                                ...
                                $HTML1 .= $HTML;
                                $HTML = '';
                                $rpt_stmt = "select vicidial_carrier_log.*, vicidial_log.phone_number from vicidial_carrier_log left join vicidial_log on vicidial_log.uniqueid=vicidial_carrier_log.uniqueid where vicidial_carrier_log.call_date>='$start $query_date_D' and vicidial_carrier_log.call_date<='$end $query_date_T' $server_ip_SQL order by vicidial_carrier_log.call_date asc";
                                ...
                                $HTML.="<th colspan='12'><font size='2'>" . _QXZ("CARRIER LOG RECORDS FOR") . " $start $query_date_D " . _QXZ("TO") . " $end $query_date_T $server_rpt_string, " . _QXZ("RECORDS") . " #$lower_limit-$upper_limit</font></th>";
//                                $HTML.="<th><a href=\"$PHP_SELF?SUBMIT=$SUBMIT&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS&lower_limit=$lower_limit&upper_limit=$upper_limit&file_download=1\">[" . _QXZ("DOWNLOAD") . "]</a></th>";
                                $HTML.="</tr>";
                                $HTML.="<tr>";
                                ...
                                for ($i = 1; $i <= mysqli_num_rows($rpt_rslt); $i++) {
                                    ...
                                    if (strlen($row["phone_number"]) == 0) {
                                        ...
                                        while ($row2 = mysqli_fetch_array($rslt2)) {
                                            if (strlen($row2["alt_phone"]) >= 7 && preg_match("/$row2[alt_phone]/", $channel)) {
                                                ...
                                                $phone_note = "ALT";
                                            } else if (strlen($row2["address3"]) >= 7 && preg_match("/$row2[address3]/", $channel)) {
                                                $phone_number = $row2["address3"];
                                                $phone_note = "ADDR3";
                                            } else if (strlen($row2["phone_number"]) >= 7 && preg_match("/$row2[phone_number]/", $channel)) {
                                    ...
                                    if ($i >= $lower_limit && $i <= $upper_limit) {
                                        if (strlen($row["channel"]) > 37) {
                                            $row["channel"] = substr($row["channel"], 0, 37) . "...";
                                        }
                                        ...
                                        $HTML.="<tr>";
                                        $HTML.="<td><font size='2'>" . $row["uniqueid"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["call_date"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["server_ip"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["lead_id"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["hangup_cause"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["dialstatus"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["channel"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["dial_time"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["answered_time"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["sip_hangup_cause"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $row["sip_hangup_reason"] . "</font></td>";
                                        $HTML.="<td><font size='2'>" . $phone_number . "</font></td>";
                                        $HTML.="</tr>\n";
                                    }
                                }
                                $carrier_rpt.="+----------------------+---------------------+-----------------+-----------+--------------+-------------+------------------------------------------+-----------+---------------+----------+--------------------------------+--------------+\n";
                                ...
                                if ($ll >= 1) {
                                    ...
                                    $HTML.="<th colspan='6'><font size='2'><a href=\"?SUBMIT=$SUBMIT&start=$start&end=$end&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T&report_display_type=$report_display_type$server_ipQS&lower_limit=$ll\">[<<< " . _QXZ("PREV") . " 1000 " . _QXZ("records") . "]</a></font></th>";
                                } else {
                                ...
                                if (($lower_limit + 1000) < mysqli_num_rows($rpt_rslt)) {
                                    ...
                                    $HTML.="<td align='right' colspan='6'><font size='2'><a href=\"?SUBMIT=$SUBMIT&start=$start&end=$end&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T&report_display_type=$report_display_type$server_ipQS&lower_limit=" . ($lower_limit + 1000) . "\">[" . _QXZ("NEXT") . " $max_limit " . _QXZ("records") . " >>>]</a></font></th>";
                                } else {
                                    $carrier_rpt_hf.=sprintf("%23s", " ");
                                    $HTML.="<td colspan='6'>&nbsp;</td>";
                                }
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title">Dial Log Report Search</h5>
            ...
            <form method="get" autocomplete="off">
                <div class="modal-body">
                    <input type="hidden" name="report_display_type" value="HTML"/>
                    <div class="form-group">
                        <label for="example_input_full_name">Start Date:</label>
                        <input type="text" class="form-control datepicker" placeholder="" name="start" value="<?php echo @$_GET['start']; ?>"/>
                    </div>
                    <div class="form-group">
                        <label for="example_input_full_name">End Date:</label>
                        <input type="text" class="form-control datepicker" placeholder="" name="end" value="<?php echo @$_GET['end']; ?>"/>
                    </div>
                    <div class="form-group">
                        <label for="example_input_full_name">Server IP</label>
                        <select name="server_ip[]" multiple="" class="form-control select2" style="width:100%;">
                        <?php
                        if (preg_match('/\-\-ALL\-\-/', $server_ip_string)) {
                            echo "<option value=\"--ALL--\" selected>-- " . _QXZ("ALL SERVERS") . " --</option>\n";
                        } else {
                            echo "<option value=\"--ALL--\">-- " . _QXZ("ALL SERVERS") . " --</option>\n";
                        }
                        $o = 0;
                        while ($servers_to_print > $o) {
                            if (preg_match("/\|$LISTserverIPs[$o]\|/", $server_ip_string)) {
                                echo "<option selected value=\"$LISTserverIPs[$o]\">$LISTserverIPs[$o] - $LISTserver_names[$o]</option>\n";
                            } else {
                                echo "<option value=\"$LISTserverIPs[$o]\">$LISTserverIPs[$o] - $LISTserver_names[$o]</option>\n";
                            }
                            $o++;
                        }
                        ?>
                ...
                <div class="modal-footer modal-footer-uniform">
                    ...
                    <button type="submit" class="btn btn-bold btn-pure btn-primary float-right">Search</button>
                </div>
            </form>
        </div>
    </div>

...
// 	$Bline  = "$HTbegin$HTstation$HTphone$HTuser$HTusergroup$HTsessionid$HTlisten$HTbarge$HTwhisper$HTstatus$HTpause$HTcustphone$HTserver_ip$HTcall_server_ip$HTtime$HTcampaign$HTcalls$HTigcall\n";
        ...
        if($monitor_active == 'MONITOR'){
            ...
            $Bline  = "$HTuser$HTcampaign$HTusergroup$HTstatus$HTtime$HTcalls$HTigcall$Monitor\n";   
        }elseif($monitor_active == 'BARGE'){
            $Monitor = '<th>Action</th>';
            $Bline  = "$HTuser$HTcampaign$HTusergroup$HTstatus$HTtime$HTcalls$HTigcall$Monitor\n";   
        }else{
            $Bline  = "$HTuser$HTcampaign$HTusergroup$HTstatus$HTtime$HTcalls$HTigcall\n";   
        }
...
#				{$G='<SPAN class="orange"><B>'; $EG='</B></SPAN>';}
		...
		if ($report_display_type=='HTML')
			...
			if ( (strlen($monitor_phone)>1) and (preg_match("/MONITOR|BARGE|WHISPER/",$monitor_active) ) )
				{$L1="<a href=\"javascript:send_monitor('$Lsessionid','$Lserver_ip','MONITOR');\" class='btn btn-success btn-app'><i class='fa fa-headphones'></i></a>";   $R='';}
			if ( (strlen($monitor_phone)>1) and (preg_match("/BARGE|WHISPER/",$monitor_active) ) )
				{$R=" <td NOWRAP align=center> <a href=\"javascript:send_monitor('$Lsessionid','$Lserver_ip','BARGE');\">"._QXZ("BARGE")."</a></td>";}
			if ( (strlen($monitor_phone)>1) and (preg_match("/BARGE|WHISPER/",$monitor_active) ) )
				{$R1="<a href=\"javascript:send_monitor('$Lsessionid','$Lserver_ip','BARGE');\" class='btn btn-warning btn-app'><i class='fa fa-microphone'></i></a>";}
...
//				$Aecho .= "<tr class='$tr_class'><td NOWRAP> $G$extension$EG$Aring_note[$i]</td><td NOWRAP>$phoneD<a href=\"./user_status.php?user=$Luser\" target=\"_blank\">$G$user$EG</a> <a href=\"javascript:ingroup_info('$Luser','$j');\">$G+$EG</a> </td><td NOWRAP align=right>$UGD $G$sessionid$EG$L$R </td><td NOWRAP align=center> $G"._QXZ("$status",6)."$EG</td><td bgcolor=white align=center><font class='Hblank'>$CM</td>$pausecodeHTML<td NOWRAP align=right>$CP$SVD$G$call_time_MS$EG </td><td NOWRAP align=right> $G$campaign_id$EG </td><td NOWRAP align=right> $G$calls_today$EG </td>$INGRP</tr>\n";
				switch(trim($status)){
                            ...
                            if($monitor_active == 'MONITOR'){
                                $Aecho .= "<tr class='$tr_class'><td>$user $UserID</td><td NOWRAP align=right>$campaign_id</td><td>$Auser_group[$j]</td><td NOWRAP align=center>"._QXZ("$status",6).$pausecode."</td><td NOWRAP align=right>$call_time_MS </td><td NOWRAP align=right>$calls_today </td>$INGRP<td>$L1</td></tr>\n";
//                                
                            }elseif($monitor_active == 'BARGE'){
                                $Aecho .= "<tr class='$tr_class'><td>$user $UserID</td><td NOWRAP align=right>$campaign_id</td><td>$Auser_group[$j]</td><td NOWRAP align=center>"._QXZ("$status",6).$pausecode."</td><td NOWRAP align=right>$call_time_MS</td><td NOWRAP align=right>$calls_today </td>$INGRP<td>$L1$R1</td></tr>\n";
                            }else{
                               $Aecho .= "<tr class='$tr_class'><td>$user $UserID</td><td NOWRAP align=right>$campaign_id</td><td>$Auser_group[$j]</td><td NOWRAP align=center>"._QXZ("$status",6).$pausecode."</td><td NOWRAP align=right>$call_time_MS</td><td NOWRAP align=right>$calls_today</td>$INGRP</tr>\n"; 
                            }

...
<!--   <section class="content-header">
        <h1>
            Campaign Inbound Status Summary Report
        </h1>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="<?php echo base_url('dashboard');?>"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="breadcrumb-item active">Campaign Inbound Status Summary Report</li>
        </ol>
    </section>-->
    ...
    <section class="content">
        ...
        $data = $database->query("select a.campaign_id,group_concat(a.status) as nameStatus,group_concat(a.count) as countStatus
from (SELECT campaign_id,status,count(*) as count FROM asterisk.vicidial_closer_log  where call_date BETWEEN '" . $start . " 00:00:00' AND '" . $end . " 23:59:59' group by campaign_id,status) a group by a.campaign_id")->fetchAll(PDO::FETCH_ASSOC);
        ?>
        <!-- Default box -->
        <div class="panel">
            <div class="panel-heading">
                <span class="panel-title"><i class="fa fa-list-alt"></i> Campaign Inbound Status Summary Report</span>
                    ...
                    <table id="table-list-campaigns" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            <?php foreach ($data as $value) { ?>
                                <tr>
                                    <th><?php echo $value['campaign_id']; ?></th>
                                <?php
                                foreach ($StatusListings as $status) {
    ...
    $(function () {
        ...
        $('#daterange-btn').daterangepicker(
                ...
                function (start, end) {
                    ...
                    window.location.href = "<?php echo base_url('campaigns/inbound_status');?>?start=" + start + "&end=" + end;
                }
        );
    })

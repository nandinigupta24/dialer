...
$UserGroupListings = $database->select('vicidial_user_groups', ['user_group', 'group_name']);
...
if($_SESSION['CurrentLogin']['user_group'] == 'ADMIN'){
    ...
    $stmt = "select campaign_id,campaign_name from vicidial_campaigns where active='Y' AND user_group IN ('".implode("','",$_SESSION['CurrentLogin']['allowed_teams_access'])."') order by campaign_id;";
...
if(!empty($_SESSION['CurrentLogin']['user_group']) && $_SESSION['CurrentLogin']['user_group'] != 'ADMIN'){
    $stmt = "select user_group, group_name from vicidial_user_groups where user_group IN ('".implode("','",$_SESSION['CurrentLogin']['allowed_teams_access'])."') order by user_group;";

$i = 0;

}else{
//         $stmt = "select user_group, group_name from vicidial_user_groups where user_group = '".$_SESSION['CurrentLogin']['user_group']."' order by user_group;";
...
} $select_list .= ">5 " . _QXZ("seconds") . "</option>";
...
<HTML>
    <HEAD>
        <link rel="icon" href="<?php echo base_url_reports('assets/images/favicon.ico');?>"/>
        <script language="Javascript">
            ...
            function send_monitor(session_id, server_ip, stage)
            {
                ...
                if (xmlhttp)
                {
                    ...
                    xmlhttp.open('POST', '<?php echo $baseURL;?>agent/non_agent_api.php');
...
//                            console.log($('.chart-responsive').data('var'));
                                    var arrayNew12 = [];
                                    var arrayNew = '';
//                                    var keys = [];
                                    var ArrayValues = [];
                                    var ArrayValues1 = '';
                                $('.CarrierStatsDetails').each(function(){
                                    arrayNew121 = [];
                                     arrayNew121['y'] = $(this).data('id');
                                    arrayNew1 = "{y:'"+$(this).data('id')+"',";
                                    $(this).find('.CarrierDetail').each(function(){
                                        arrayNew1 = arrayNew1+$(this).data('id')+":"+$(this).data('val')+",";
                                        var keyField = $(this).data('id');
                                        var keyValue = $(this).data('val');
                                        arrayNew121[keyField] = keyValue;
                                        if (!ArrayValues.includes($(this).data('id'))){
                                            ArrayValues.push($(this).data('id'));
                                            ArrayValues1 = ArrayValues1+"'"+$(this).data('id')+"',";
                                        }
                                    });

                                    arrayNew1 = arrayNew1.replace(/,\s*$/, "")+'},';
                                    arrayNew = arrayNew+arrayNew1;
                                    arrayNew12.push(arrayNew121);
                                });
                                        ArrayValues1 = ArrayValues1.replace(/,\s*$/, "");
                                        arrayNew = arrayNew.replace(/,\s*$/, "");
                                 ...
                                 $(function () {
                                        ...
                                        var bar = new Morris.Bar({
                                            ...
                                            resize: true,
                                            data: arrayNew12,
                                            barColors: ['#745af2', '#fc4b6c', '#06d79c','#9B3FD3','#3FD3B1','#DC3015'],
                                            barSizeRatio: 0.5,
                                            barGap: 5,
                                            xkey: 'y',
                                            ykeys: ArrayValues,
...
//                    alert(RR);
        ...
        ?>
                    ...
                    var selCampObj = document.getElementById('Campaign-Selection');
                    var i;
                    var count = 0;
                    var selected_all = 0;
                    ...
                    var selCampObj = document.getElementById('Group-Selection');
                    var i;
                    var count = 0;
                    var selected_all = 0;
                    for (i = 0; i < selCampObj.options.length; i++)
...
console.log(reload_url);
        ...
        <STYLE type="text/css">
            ...
            tr.TRblank {background-color: white;color:black;}
...
echo "<TABLE CELLPADDING=4 CELLSPACING=0 style='display:none;'><TR><TD>";
...
echo " &nbsp; &nbsp; &nbsp; <font class='top_settings_val'>" . _QXZ("refresh") . ": <span id=refresh_countdownOLD name=refresh_countdown></span></font>\n\n";
...
//echo "<span id=realtime_contentOLD name=realtime_content></span>\n";
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <div class="panel panel-fullscreen">
                    ...
                    <div class="panel-heading"><span class="panel-title"><i class="fa fa-dashboard"></i> Realtime Dashboard </span>

                        <ul class="nav panel-tabs">

                            <li><a href="" onclick="" title="Refresh Stats"><span id="refresh_countdown" class="text-default"></span></a></li>

                            <li class="FilterTab active" data-title='Dashboard'><a href="javascript:void(0);" data-toggle="tab"><i class="fa fa-dashboard"></i></a></li>

                            <li class="FilterTab" data-title='Dialer'><a href="javascript:void(0);" title="Dialler Overview"><i class="fa fa-sliders"></i></a></li>

                            <li class="FilterTab" data-title='Agent'><a href="javascript:void(0);" title="Agent Overview"><i class="fa fa-user"></i></a></li>

                            <li class="FilterTab" data-title='Stats'><a href="javascript:void(0);" title="Average Stats"><i class="fa fa-line-chart"></i></a></li>

                            <li class="FilterTab" data-title='Calls'><a href="javascript:void(0);" title="Calls &amp; Queue Overview"><i class="fa fa-phone"></i></a></li>

                            <li class="FilterTab" data-title='Carrier'><a href="javascript:void(0);" title="Carriers &amp; Queue Overview"><i class="fa fa-line-chart"></i></a></li>

                            <li class="FilterTab" data-title='Session'>
                                <a href="javascript:void(0);" title="Session ID Overview"><i class="fa fa-gavel"></i></a>
                            </li>

                            <li><a href="#modal-default" data-toggle="modal"><i class="fa fa-search"></i></a></li>

                            <li><a href="#monitor-modal" data-toggle="modal"><i class="fa fa-eye" title="Report"></i></a></li>
                        </ul>
                    ...
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12" id="realtime_content" style="margin-top:20px;">
                            </div>
                        </div>
...
<input type="hidden" name="" value="none" class="" id="DialerView"/>
<input type="hidden" name="" value="none" class="" id="AgentView"/>
<input type="hidden" name="" value="none" class="" id="StatView"/>
<input type="hidden" name="" value="none" class="" id="CallView"/>
<input type="hidden" name="" value="none" class="" id="CARRIERstatsNEW"/>
<input type="hidden" name="" value="none" class="" id="SessionView"/>
<?php
require 'modal.php';
require 'footer.php';
?>
...
<script>
    $(document).on('click', '.FilterTab', function () {
        ...
        if($(this).hasClass('active')){
            ...
            switch(title){
            case 'Dialer':
                $('.DialerLog').children().hide();
                $('#DialerView').val('none');
                break;
            case 'Agent':
                $('.AgentLog').children().hide();
                $('#AgentView').val('none');
                break;
            case 'Stats':
                $('.StatsLog').children().hide();
                $('#StatView').val('none');
                break;
            case 'Calls':
                $('.CallLog').children().hide();
                $('#CallView').val('none');
                break;
            case 'Dashboard':
                $('.AgentLog').children().show();
                $('.CallLog').children().show();
                 $('.DialerLog').children().show();
                $('.StatsLog').children().show();
                $('.CarrierStats-TABLE').hide();
                break;
            case 'Carrier':
                $('.CarrierStats-TABLE').hide();
                $('#CARRIERstatsNEW').val('none');
                gather_realtime_content();
            break;
            case 'Session':
                $('#SessionView').val('none');
                gather_realtime_content();
            break;
            default:
            ...
            case 'Dialer':
                $('.DialerLog').children().show();
                $('#DialerView').val('inline');
                break;
            case 'Agent':
                $('.AgentLog').children().show();
                $('#AgentView').val('inline');
                break;
            case 'Stats':
                $('.StatsLog').children().show();
                $('#StatView').val('inline');
            ...
            case 'Dashboard':
                ...
                $('.CallLog').children().show();
                 $('.DialerLog').children().show();
                $('.StatsLog').children().show();
                $('.CarrierStats-TABLE').show();
                break;
            case 'Carrier':
                $('.CarrierStats-TABLE').show();
                $('#CARRIERstatsNEW').val('block');
                gather_realtime_content();
                break;
            case 'Session':
            $('#SessionView').val('OPEN');
            break;
            default:
    ...
    (function blink() {
    $('#refresh_countdown').fadeOut(500).fadeIn(500, blink);
    })();


    function update_variables_monitor(){
        var MonitorActiveNew = $('#monitor_active_new').val();
        var MonitorExtension = $('#monitor_extension').val();
        var WithInbound = $('#with_inbound_new').val();

        $('#monitor_active').val(MonitorActiveNew);
        $('#monitor_phone').val(MonitorExtension);
        $('#with_inbound').val(WithInbound);
        $('#CloseModal').click();

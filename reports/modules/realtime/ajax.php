...
if(!empty($_SESSION['Login']['user']) && $_SESSION['Login']['user']){
...
}elseif(isset($_POST['AgentView'])){
    $AgentView = $_POST['AgentView'];
}else{
    $AgentView = 'none';
}
/*Agent View*/
if(isset($_GET['DialerView'])){
    $DialerView = $_GET['DialerView'];
}elseif(isset($_POST['DialerView'])){
    $DialerView = $_POST['DialerView'];
}else{
    $DialerView = 'none';
}
/*Agent View*/
...
}elseif(isset($_POST['StatView'])){
    $StatView = $_POST['StatView'];
}else{
    $StatView = 'none';
}
/*Agent View*/
if(isset($_GET['CallView'])){
    $CallView = $_GET['CallView'];
}elseif(isset($_POST['CallView'])){
    $CallView = $_POST['CallView'];
}else{
    $CallView = 'none';
}
if(isset($_GET['CARRIERstatsNew']) && $_GET['CARRIERstatsNew'] == 'block'){
//    $CARRIERstatsNew = $_GET['CARRIERstatsNew'];
    $CARRIERstatsNew = "";
}elseif(isset($_POST['CARRIERstatsNew']) && $_POST['CARRIERstatsNew'] == 'block'){
//    $CARRIERstatsNew = $_POST['CARRIERstatsNew'];
    $CARRIERstatsNew = "";
}else{
    $CARRIERstatsNew = "style='display:none;'";
}
...
if ($auth > 0)
	...
	$stmt="SELECT count(*) from vicidial_users where user='$PHP_AUTH_USER' and view_reports='1';";
	if ($DB) {echo "|$stmt|\n";}
	$rslt=mysql_to_mysqli($stmt, $link);
	$row=mysqli_fetch_row($rslt);
	$reports_auth=$row[0];
...
//	$LOGallowed_campaignsSQL = "and campaign_id IN('$rawLOGallowed_campaignsSQL')";
	$LOGallowed_campaignsSQL = "and campaign_id IN('".implode("','",$_SESSION['CurrentLogin']['CampaignID'])."')";
	$LOGallowed_campaignsSQL11 = "and ls.campaign_id IN('".implode("','",$_SESSION['CurrentLogin']['CampaignID'])."')";
	$whereLOGallowed_campaignsSQL = "where campaign_id IN('$rawLOGallowed_campaignsSQL')";
	}
//$regexLOGallowed_campaigns = " $LOGallowed_campaigns ";
$regexLOGallowed_campaigns = " ".implode(" ",$_SESSION['CurrentLogin']['CampaignID'])." - ";
$allactivecampaigns='';
...
if ($RTajax > 0)
	...
	function send_monitor(session_id,server_ip,stage)
		...
		if (xmlhttp)
			...
			xmlhttp.open('POST', '../../../../agent/non_agent_api.php');
...
if(!empty($groups[0]) && $groups[0] == 'ALL-ACTIVE'){
                            $CarrierCampaignID = implode("','",$_SESSION['CurrentLogin']['CampaignID']);
                        }elseif(!empty($groups[0])){
                            $CarrierCampaignID = implode("','",$groups);
                        }else{
...
//		$stmtB="SELECT dialstatus,count(*) from vicidial_carrier_log where call_date >= \"$timeTWENTYFOURhoursAGO\" group by dialstatus;";
            $stmtB="SELECT vcl.dialstatus,count(*)
FROM asterisk.vicidial_carrier_log vcl
join asterisk.vicidial_list vl
on vcl.lead_id = vl.lead_id
join asterisk.vicidial_lists ls
on vl.list_id = ls.list_id
where vcl.call_date >= '".$timeTWENTYFOURhoursAGO."' and ls.campaign_id IN ('".$CarrierCampaignID."')
group by vcl.dialstatus";
		if ($DB > 0) {echo "\n|$stmtB|\n";}
		$rslt=mysql_to_mysqli($stmtB, $link);
...
//<<<<<<< HEAD
		$CARRIERstatsHTML .= "<table class='table table-striped table-bordered' style='display:none;'>";
//=======
//		$CARRIERstatsHTML .= "<table class='table table-striped table-bordered'>";
//>>>>>>> b553a7c83c47b1f8938e0ce8d57ae24571a5b878
		$CARRIERstatsHTML .= "<thead class='bg-black' style='font-weight:600;'><tr><th colspan='7'>"._QXZ("CARRIER STATS")."</th></tr><tr>";
//		$CARRIERstatsHTML .= "<TD ALIGN=LEFT><font class='top_settings_key'>"._QXZ("CARRIER STATS").": &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; </font></TD>";
		$CARRIERstatsHTML .= "<th>"._QXZ("HANGUP STATUS")." </th>";
		$CARRIERstatsHTML .= "<th>"._QXZ("24 HOURS")." </th>";
		$CARRIERstatsHTML .= "<th>"._QXZ("6 HOURS")."</th>";
		$CARRIERstatsHTML .= "<th>"._QXZ("1 HOUR")."</th>";
		$CARRIERstatsHTML .= "<th>"._QXZ("15 MIN")."</th>";
		$CARRIERstatsHTML .= "<th>"._QXZ("5 MIN")."</th>";
		$CARRIERstatsHTML .= "<th>"._QXZ("1 MIN")."</th>";
		$CARRIERstatsHTML .= "</tr></thead><tbody>";
...
join asterisk.vicidial_lists ls
on vl.list_id = ls.list_id
where vcl.call_date >= '".$timeSIXhoursAGO."' and ls.campaign_id IN ('".$CarrierCampaignID."')
group by vcl.dialstatus";
			if ($DB > 0) {echo "\n|$stmtB|\n";}
			$rslt=mysql_to_mysqli($stmtB, $link);
...
join asterisk.vicidial_lists ls
on vl.list_id = ls.list_id
where vcl.call_date >= '".$timeONEhourAGO."' and ls.campaign_id IN ('".$CarrierCampaignID."')
group by vcl.dialstatus";
			if ($DB > 0) {echo "\n|$stmtB|\n";}
			$rslt=mysql_to_mysqli($stmtB, $link);
			$scar_to_print = mysqli_num_rows($rslt);
...
join asterisk.vicidial_lists ls
on vl.list_id = ls.list_id
where vcl.call_date >= '".$timeFIFTEENminutesAGO."' and ls.campaign_id IN ('".$CarrierCampaignID."')
group by vcl.dialstatus";
			if ($DB > 0) {echo "\n|$stmtB|\n";}
			$rslt=mysql_to_mysqli($stmtB, $link);
			$scar_to_print = mysqli_num_rows($rslt);
...
join asterisk.vicidial_lists ls
on vl.list_id = ls.list_id
where vcl.call_date >= '".$timeFIVEminutesAGO."' and ls.campaign_id IN ('".$CarrierCampaignID."')
group by vcl.dialstatus";
			if ($DB > 0) {echo "\n|$stmtB|\n";}
			$rslt=mysql_to_mysqli($stmtB, $link);
			$scar_to_print = mysqli_num_rows($rslt);
...
join asterisk.vicidial_lists ls
on vl.list_id = ls.list_id
where vcl.call_date >= '".$timeONEminuteAGO."' and ls.campaign_id IN ('".$CarrierCampaignID."')
group by vcl.dialstatus";
...
//                       echo $stmtB;
			...
			while ($print_ctp < $ctp)
				...
				$CARRIERstatsHTML .= "<tr>";
//				$CARRIERstatsHTML .= "<tr><font class='top_settings_val'>&nbsp;</TD>";
				$CARRIERstatsHTML .= "<td>".$TFhour_status[$print_ctp]."</td>";
				$CARRIERstatsHTML .= "<td>".$TFhour_count[$print_ctp]."<span class='badge'><small class='text-danger'>".sprintf("%01.1f", $TFhour_pct)."%</small></span></td>";
				$CARRIERstatsHTML .= "<td>".$SIXhour_count[$print_ctp]."<span class='badge'><small class='text-danger'>".sprintf("%01.1f", $SIXhour_pct)."%</small></td>";
				$CARRIERstatsHTML .= "<td>".$ONEhour_count[$print_ctp]."<span class='badge'><small class='text-danger'>".sprintf("%01.1f", $ONEhour_pct)."%</small></td>";
				$CARRIERstatsHTML .= "<td>".$FTminute_count[$print_ctp]."<span class='badge'><small class='text-danger'>".sprintf("%01.1f", $TFminute_pct)."%</small></td>";
				$CARRIERstatsHTML .= "<td>".$FIVEminute_count[$print_ctp]."<span class='badge'><small class='text-danger'>".sprintf("%01.1f", $FIVEminute_pct)."%</small></td>";
				$CARRIERstatsHTML .= "<td>".$ONEminute_count[$print_ctp]."<span class='badge'><small class='text-danger'>".sprintf("%01.1f", $ONEminute_pct)."%</small></td>";
				$CARRIERstatsHTML .= "</tr>";
//<<<<<<< HEAD
//=======
//
//>>>>>>> b553a7c83c47b1f8938e0ce8d57ae24571a5b878
                                $arrayNew['24 Hour'][$TFhour_status[$print_ctp]] = $TFhour_count[$print_ctp];
                                $arrayNew['6 Hour'][$TFhour_status[$print_ctp]] = $SIXhour_count[$print_ctp];
                                $arrayNew['1 Hour'][$TFhour_status[$print_ctp]] = $ONEhour_count[$print_ctp];
                                $arrayNew['15 Min'][$TFhour_status[$print_ctp]] = $FTminute_count[$print_ctp];
                                $arrayNew['5 Min'][$TFhour_status[$print_ctp]] = $FIVEminute_count[$print_ctp];
                                $arrayNew['1 Min'][$TFhour_status[$print_ctp]] = $ONEminute_count[$print_ctp];
//<<<<<<< HEAD
//=======
//
//>>>>>>> b553a7c83c47b1f8938e0ce8d57ae24571a5b878
			...
			$CARRIERstatsHTML .= "<tfoot><tr>";
//			$CARRIERstatsHTML .= "<td BGCOLOR=white><font class='top_settings_val'>&nbsp;</TD>";
			$CARRIERstatsHTML .= "<td>"._QXZ("TOTALS")."</td>";
			$CARRIERstatsHTML .= "<td>".($TFhour_total+0)."</td>";
			$CARRIERstatsHTML .= "<td> ".($SIXhour_total+0)."</td>";
			$CARRIERstatsHTML .= "<td>".($ONEhour_total+0)."</td>";
			$CARRIERstatsHTML .= "<td>".($FTminute_total+0)."</td>";
			$CARRIERstatsHTML .= "<td> ".($FIVEminute_total+0)."</td>";
			$CARRIERstatsHTML .= "<td>".($ONEminute_total+0)."</td>";
			$CARRIERstatsHTML .= "</tr></tfoot>";
...
$D_active_chats=0;
$chats_waiting=0;
//pr($_SESSION['CurrentLogin']['CampaignID']);
//exit;
$SaleStatus = $database->select('vicidial_campaign_statuses','status',['Sale'=>'Y']);
if(!empty($_SESSION['CurrentLogin']['user_group']) && $_SESSION['CurrentLogin']['user_group'] == 'ADMIN'){
    $CampaignIDList  = $database->select('vicidial_campaigns','campaign_id');
}else{
    $CampaignIDList  = $database->select('vicidial_campaigns','campaign_id',['user_group'=>$_SESSION['CurrentLogin']['user_group']]);
}

if($group == 'ALL-ACTIVE'){
$SaleCount = $database->count('vicidial_agent_log',['AND'=>['event_time[>=]'=>date('Y-m-d').' 00:00:00','campaign_id'=>$CampaignIDList,'status'=>$SaleStatus]]);
}else{
$SaleCount = $database->count('vicidial_agent_log',['AND'=>['event_time[>=]'=>date('Y-m-d').' 00:00:00','campaign_id'=>$group,'status'=>$SaleStatus]]);
}
...
if ($report_display_type=='HTML')
	{
	$Cecho = '';
	$Cecho .= "<table class='table table-bordered table-striped InboundCalls'>";
//	$Cecho .= "<thead class='bg-black' style='background-color:#ea5218;color:#fff;font-weight:600;'><tr>";
	$Cecho .= "<thead><tr class='bg-black' style='font-weight:600;'>";
	$Cecho .= "<td colspan=5>"._QXZ("Calls Waiting")."</td>";
	$Cecho .= "<td colspan=2>$NOW_TIME</td>";
	$Cecho .= "</tr>\n";
	$Cecho .= "<tr class='bg-primary' style='font-weight:600;'>";
	$Cecho .= "<td>"._QXZ("STATUS")."</td>";
	$Cecho .= "<td>"._QXZ("CAMPAIGN")."</td>";
	$Cecho .= "<td>"._QXZ("PHONE NUMBER")."</td>";
	$Cecho .= "<td>"._QXZ("SERVER IP")."</td>";
	$Cecho .= "<td>"._QXZ("DIALTIME")."</td>";
	$Cecho .= "<td>"._QXZ("CALL TYPE")."</td>";
	$Cecho .= "<td>"._QXZ("PRIORITY")."</td>";
//	$Cecho .= "<td>$agentonlyheader</td>";
	$Cecho .= "</tr></thead><tbody class='bg-red-new' style='color:#fff;'>\n";
	}
...
if ($report_display_type=='TEXT1')
	{
	$Aecho = '';
	$Aecho .= ""._QXZ("Agents Time On Calls Campaign").": $group_string            $NOW_TIME\n";
	}
if ($report_display_type=='HTML' || $report_display_type=='TEXT')
	{
	$Aecho = '';
	$Aecho .= "<table class='table table-bordered table-striped CallList'>";
//	$Aecho .= "<thead class='' style='background-color:#ea5218;color:#fff;font-weight:600;'>";
	$Aecho .= "<thead class='bg-black' style='font-weight:600;'><tr>";
//	$Aecho .= "<td colspan=4>"._QXZ("Agents Time On Calls Campaign").":</font></td>";
//	$Aecho .= "<td colspan=2>$NOW_TIME</td>";
//	$Aecho .= "</tr>";
...
//	$HTstation =		"<td NOWRAP><font class='top_head_key'>&nbsp; "._QXZ("STATION")." </td>";
	$HDphone =		"";
	$HTphone =		"<td>&nbsp;<a href=\"$PHP_SELF?$usergroupQS$groupQS&RR=$RR&DB=$DB&adastats=$adastats&SIPmonitorLINK=$SIPmonitorLINK&IAXmonitorLINK=$IAXmonitorLINK&usergroup=$usergroup&UGdisplay=$UGdisplay&UidORname=$UidORname&orderby=$phoneord&SERVdisplay=$SERVdisplay&CALLSdisplay=$CALLSdisplay&PHONEdisplay=$PHONEdisplay&CUSTPHONEdisplay=$CUSTPHONEdisplay&with_inbound=$with_inbound&monitor_active=$monitor_active&monitor_phone=$monitor_phone&ALLINGROUPstats=$ALLINGROUPstats&DROPINGROUPstats=$DROPINGROUPstats&NOLEADSalert=$NOLEADSalert&CARRIERstats=$CARRIERstats&PRESETstats=$PRESETstats&AGENTtimeSTATS=$AGENTtimeSTATS&INGROUPcolorOVERRIDE=$INGROUPcolorOVERRIDE&droppedOFtotal=$droppedOFtotal&report_display_type=$report_display_type\"><font class='top_head_key'>"._QXZ("PHONE")."</a>&nbsp;</td>";
	if ($RTajax > 0)
...
//	$HDsessionid =		"";
	...
	if ($RTajax > 0)
		{$HTcampaign =	"<td>Campaign</td>";}
    $HTPhone ="<td>Phone Number</td>";
	$HDcalls =			"";
        if(!empty($_POST['SessionID']) && $_POST['SessionID'] == 'OPEN'){
            $HTcalls =			"<td>Call</td><td>Session ID</td>";
        }else{
            $HTcalls =			"<td>Call</td>";
        }
//	$HTcalls =			"<td>Call</td>";
...
//	$HTpause =	'';
//	$HDigcall =			"";
//	$HTigcall =			"<td NOWRAP><font class='top_head_key'>&nbsp; "._QXZ("HOLD")." </td><td NOWRAP><font class='top_head_key'>&nbsp; "._QXZ("IN-GROUP",8)." </td></tr></thead>";
	$HTigcall =			"<td>"._QXZ("IN-GROUP",8)."</td>";

	if ($agent_pause_codes_active > 0)
		{
		$HDstatus =			"";
		$HTstatus =			"<td>Status</td>";
//		$HDpause =			"";
...
// 	$Bline  = "$HTbegin$HTstation$HTphone$HTuser$HTusergroup$HTsessionid$HTlisten$HTbarge$HTwhisper$HTstatus$HTpause$HTcustphone$HTserver_ip$HTcall_server_ip$HTtime$HTcampaign$HTcalls$HTigcall\n";
        ...
        if($monitor_active == 'MONITOR'){
            $Monitor = '<th>Action</th>';
            $Bline  = "$HTuser$HTcampaign$HTPhone$HTusergroup$HTstatus$HTtime$HTcalls$HTigcall$Monitor\n";
        }elseif($monitor_active == 'BARGE'){
            $Monitor = '<th>Action</th>';
            $Bline  = "$HTuser$HTcampaign$HTPhone$HTusergroup$HTstatus$HTtime$HTcalls$HTigcall$Monitor\n";
        }elseif($monitor_active == 'WHISPER'){
            $Monitor = '<th>Action</th>';
            $Bline  = "$HTuser$HTcampaign$HTPhone$HTusergroup$HTstatus$HTtime$HTcalls$HTigcall$Monitor\n";
        }elseif($monitor_active == 'ALL'){
            $Monitor = '<th>Action</th>';
            $Bline  = "$HTuser$HTcampaign$HTPhone$HTusergroup$HTstatus$HTtime$HTcalls$HTigcall$Monitor\n";
        }else{
            $Bline  = "$HTuser$HTcampaign$HTPhone$HTusergroup$HTstatus$HTtime$HTcalls$HTigcall\n";
        }
	...
	while ($j < $talking_to_print)
		...
		if ( ($Lstatus=='INCALL') or ($Lstatus=='PARK') )
			...
			if ($comments == 'CHAT') {$G='<SPAN class="red"><B>'; $EG='</B></SPAN>'; $status='CHAT'; $tr_class='TRred';}
				...
				{
				if ($call_time_S >= 30) {$G='<SPAN class="thistle"><B>'; $EG='</B></SPAN>'; $tr_class='TRthistle'; $TRClassNEW = 'call-orange1';}
				if ($call_time_S >= 60) {$G='<SPAN class="violet"><B>'; $EG='</B></SPAN>'; $tr_class='TRviolet'; $TRClassNEW = 'call-orange2';}
				if ($call_time_S >= 300) {$G='<SPAN class="purple"><B>'; $EG='</B></SPAN>'; $tr_class='TRpurple'; $TRClassNEW = 'call-orange3';}
...
#				{$G='<SPAN class="orange"><B>'; $EG='</B></SPAN>';}
    ...
    if($Alead_id[$i] != 0) {
      $stmtList="SELECT phone_number from vicidial_list where lead_id = $Alead_id[$i];";
      $rsltList=mysql_to_mysqli($stmtList,$link);
      $rowList=mysqli_fetch_row($rsltList);
      $call_phone = $rowList[0];
    } else {
      $call_phone = '';
    }
		...
		if ($report_display_type=='HTML')
			...
			if ($SIPmonitorLINK>0) {$L1="<a href=\"sip:0$Lsessionid@$server_ip\">"._QXZ("LISTEN")."</a>";   $R='';}
			if ($IAXmonitorLINK>0) {$L="<td NOWRAP align=center> <a href=\"iax:0$Lsessionid@$server_ip\">"._QXZ("LISTEN")."</a></td>";   $R='';}
			if ($IAXmonitorLINK>0) {$L1="<a href=\"iax:0$Lsessionid@$server_ip\">"._QXZ("LISTEN")."</a>";   $R='';}
			if ($SIPmonitorLINK>1) {$R=" <td NOWRAP align=center> <a href=\"sip:$Lsessionid@$server_ip\">"._QXZ("BARGE")."</a></td>";}
			if ($SIPmonitorLINK>1) {$R1="<a href=\"sip:$Lsessionid@$server_ip\">"._QXZ("BARGE")."</a>";}
			if ($IAXmonitorLINK>1) {$R=" <td NOWRAP align=center> <a href=\"iax:$Lsessionid@$server_ip\">BARGE</a></td>";}
			if ($IAXmonitorLINK>1) {$R1="<a href=\"iax:$Lsessionid@$server_ip\">BARGE</a>";}
			if ( (strlen($monitor_phone)>1) and (preg_match("/MONITOR|BARGE|WHISPER/",$monitor_active) ) )
				{$L="<td NOWRAP align=center> <a href=\"javascript:send_monitor('$Lsessionid','$Lserver_ip','MONITOR');\">"._QXZ("LISTEN")."</a></td>";   $R='';}
			if ( (strlen($monitor_phone)>1) and (preg_match("/MONITOR|BARGE|WHISPER/",$monitor_active) ) )
				{$L1="<a href=\"javascript:send_monitor('$Lsessionid','$Lserver_ip','MONITOR');\" class='btn btn-success btn-app' data-toggle='tooltip' title='Monitor'><i class='fa fa-headphones'></i></a>";   $R='';}
			if ( (strlen($monitor_phone)>1) and (preg_match("/BARGE|WHISPER/",$monitor_active) ) )
				{$R=" <td NOWRAP align=center> <a href=\"javascript:send_monitor('$Lsessionid','$Lserver_ip','BARGE');\">"._QXZ("BARGE")."</a></td>";}
			if ( (strlen($monitor_phone)>1) and (preg_match("/BARGE|WHISPER/",$monitor_active) ) )
				{$R1="<a href=\"javascript:send_monitor('$Lsessionid','$Lserver_ip','BARGE');\" class='btn btn-warning btn-app' data-toggle='tooltip' title='Barge'><i class='fa fa-microphone'></i></a>";}
...
//                            $R=" <td NOWRAP align=center> <a href=\"javascript:send_monitor('$Lsessionid','$Lserver_ip','WHISPER');\">WHISPER</a></td>";
                            $R="<a href=\"javascript:send_monitor('$Lsessionid','$Lserver_ip','WHISPER');\" class='btn btn-danger btn-app' data-toggle='tooltip' title='Whisper'><i class='fa fa-bullhorn'></i></a>";
...
//				$Aecho .= "<tr class='$tr_class'><td NOWRAP> $G$extension$EG$Aring_note[$i]</td><td NOWRAP>$phoneD<a href=\"./user_status.php?user=$Luser\" target=\"_blank\">$G$user$EG</a> <a href=\"javascript:ingroup_info('$Luser','$j');\">$G+$EG</a> </td><td NOWRAP align=right>$UGD $G$sessionid$EG$L$R </td><td NOWRAP align=center> $G"._QXZ("$status",6)."$EG</td><td bgcolor=white align=center><font class='Hblank'>$CM</td>$pausecodeHTML<td NOWRAP align=right>$CP$SVD$G$call_time_MS$EG </td><td NOWRAP align=right> $G$campaign_id$EG </td><td NOWRAP align=right> $G$calls_today$EG </td>$INGRP</tr>\n";
				switch(trim($status)){
                                    case 'INCALL':
                                        ...
                                        break;
                                    case 'PAUSED':
                                        $tr_class = 'call-yelllow';
                                        break;
                                    case 'CLOSER':
                                        $tr_class = 'call-orange';
                                        break;
                                    case 'READY':
                                        if($call_time_S <= 30){
                                            $tr_class = 'call-orange';
                                        }elseif($call_time_S <= 60){
                                            $tr_class = 'call-orange1';
                                        }elseif($call_time_S <= 300){
                                            $tr_class = 'call-orange2';
                                        }else{
                                            $tr_class = 'call-orange3';
                                        }
                                ...
                                if(!empty(trim($pausecode)) && trim($pausecode)){
                                    $pausecode = ' - '.$pausecode;
                                }
                                $SessionIDHTML = '';
                                if(!empty($_POST['SessionID']) && $_POST['SessionID'] == 'OPEN'){
                                    $SessionIDHTML = '<td>'.$Lsessionid.'</td>';
                                }

                            if($monitor_active == 'MONITOR'){
                                $Aecho .= "<tr class='$tr_class'><td>$user $UserID</td><td NOWRAP align=right>$campaign_id</td><td NOWRAP align=right>$call_phone</td><td>$Auser_group[$j]</td><td NOWRAP align=center>"._QXZ("$status",6).$pausecode. (trim($CM)?"($CM)":'')."</td><td NOWRAP align=right>$call_time_MS </td><td NOWRAP align=right>$calls_today </td>$SessionIDHTML$INGRP<td>$L1</td></tr>\n";
//
                            }elseif($monitor_active == 'BARGE'){
                                $Aecho .= "<tr class='$tr_class'><td>$user $UserID</td><td NOWRAP align=right>$campaign_id</td><td NOWRAP align=right>$call_phone</td><td>$Auser_group[$j]</td><td NOWRAP align=center>"._QXZ("$status",6).$pausecode. (trim($CM)?"($CM)":'')."</td><td NOWRAP align=right>$call_time_MS</td><td NOWRAP align=right>$calls_today </td>$SessionIDHTML$INGRP<td>$L1$R1</td></tr>\n";
                            }elseif($monitor_active == 'WHISPER'){
                               ...
                               $Aecho .= "<tr class='$tr_class'><td>$user $UserID</td><td NOWRAP align=right>$campaign_id</td><td NOWRAP align=right>$call_phone</td><td>$Auser_group[$j]</td><td NOWRAP align=center>"._QXZ("$status",6).$pausecode. (trim($CM)?"($CM)":'')."</td><td NOWRAP align=right>$call_time_MS</td><td NOWRAP align=right>$calls_today</td>$SessionIDHTML$INGRP</tr>\n";
                            }
...
//	$Aecho .= "  <SPAN class=\"black\"><B>          </SPAN> - "._QXZ("Agent on a dead call")."</B>\n";
	...
	if ( ($report_display_type=='TEXT') or ($report_display_type=='WALL_1') )
		{
            require 'agent_log.php';
//		echo "$NFB$agent_total$NFE "._QXZ("agents logged in")." 1222&nbsp; &nbsp; &nbsp; &nbsp; \n";

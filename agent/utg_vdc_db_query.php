...
if (isset($_GET["CallBackPhone"]))				{$CallBackPhone=$_GET["CallBackPhone"];}
	elseif (isset($_POST["CallBackPhone"]))		{$CallBackPhone=$_POST["CallBackPhone"];}
	else {$CallBackPhone = '';}
...
$CallBackPhone = preg_replace('/[^-_0-9a-zA-Z]/','',$CallBackPhone);
...
//				$LOGallowed_campaignsSQL = "and campaign_id IN('$LOGallowed_campaignsSQL')";
				$LOGallowed_campaignsSQL = "and campaign_id IN('".implode("','",$_SESSION['CurrentLogin']['Campaign'])."')";
				}
...
if ($ACTION == 'manDiaLnextCaLL')
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1)  || (strlen($ext_context)<1) )
		...
		if ($affected_rows > 0)
			...
			##### grab the data from vicidial_list for the lead_id
			$stmt="SELECT lead_id,entry_date,modify_date,status,user,vendor_lead_code,source_id,list_id,gmt_offset_now,called_since_last_reset,phone_code,phone_number,title,first_name,middle_initial,last_name,address1,address2,address3,city,state,province,postal_code,country_code,gender,date_of_birth,alt_phone,email,security_phrase,comments,called_count,last_local_call_time,rank,owner,entry_list_id,tag FROM vicidial_list where lead_id='$lead_id' LIMIT 1;";
			$rslt=mysql_to_mysqli($stmt, $link);
...
//                        Lead Custom Data START Here
                        if(!empty($lead_id)){
                        $LeadDetail = $database->get('vicidial_list', '*', ['lead_id' =>$lead_id]);

                        $CustomFields = $database->get('custom_fields', '*', ['list_id'=>$list_id]);
												$CustomFieldsCount = $database->count('custom_fields', '*', ['list_id'=>$list_id]);

                        unset($CustomFields['list_id']);
												$response = '';

												if($CustomFieldsCount) {
													$CustomFields = array_filter($CustomFields);
													$CustomFieldsData = $database->get('custom_fields_data', array_keys($CustomFields), ['AND' => ['list_id' => $list_id, 'lead_id' => $lead_id]]);


	                        foreach($CustomFieldsData as $k=>$v){
	                            $response .= $k.'-'.$v.'-'.$CustomFields[$k].':';
...
if ($ACTION == 'updateDISPO')
	...
	if ( (strlen($dispo_choice)<1) || (strlen($lead_id)<1) )
	...
	else
		{
                ...
                /*START*/
                get_stats_log($database, $DBUTG, $user, $campaign,'on_sale',$dispo_choice);
                /*END*/
...
//                        if(!empty($_POST['Tag']) && $_POST['Tag']){
                        $stmt="UPDATE vicidial_list set tag='".@$_POST['Tag']."', status='$dispo_choice', user='$user' $commentsSQL where lead_id='$lead_id';";
//                        }else{
...
//                        }
	...
	if ($auto_dial_level < 1)
		...
		$stmt="INSERT INTO vicidial_agent_log SET user='$user',server_ip='$server_ip',event_time='$NOW_TIME',campaign_id='$campaign',pause_epoch='$StarTtime',pause_sec='0',sub_status='$status',wait_epoch='$StarTtime',user_group='$user_group',pause_type='AGENT'$MAN_insert_leadIDsql;";
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_to_mysqli($stmt, $link);
	...
	if ( ($dispo_choice == 'CBHOLD') and (strlen($CallBackDatETimE)>10) )
		...
		$comments = preg_replace('/;/i','',$comments);
		$comments = preg_replace("/\\\\/i",' ',$comments);
                $CallBackDatETimE = date('Y-m-d H:i:s',strtotime($CallBackDatETimE));
				$stmt="INSERT INTO vicidial_callbacks (lead_id,list_id,campaign_id,phone_number,status,entry_time,callback_time,user,recipient,comments,user_group,lead_status) values('$lead_id','$list_id','$campaign','$CallBackPhone','ACTIVE','$NOW_TIME','$CallBackDatETimE','$user','$recipient','$comments','$user_group','$CallBackLeadStatus');";
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_to_mysqli($stmt, $link);
...
if ($ACTION == 'updateLEAD')
	...
	if ( ( (strlen($phone_number)<1) and (strlen($email)<1) ) or (strlen($lead_id)<1) )
	...
	else
		{
            ...
            /***********************/
            $ResultCount = $database->count('custom_fields_data',['lead_id'=>$lead_id]);
            if ($ResultCount > 0){
                $arrayInsert = [];

                for ($i = 1; $i <= 50; $i++) {
                    if(!empty($_POST['custom_'.$i])){
                        $arrayInsert["custom_".$i] = $_POST['custom_'.$i];
                    }
                }
                $CampaignDialList = $database->get('vicidial_campaigns','manual_dial_list_id',['campaign_id'=>$campaign]);
                $database->update('custom_fields_data',$arrayInsert,['AND'=>['lead_id'=>$lead_id,'list_id'=>$CampaignDialList]]);
            }else{

                $arrayInsert = [];
                $arrayInsert['lead_id'] = $lead_id;
                $arrayInsert['list_id'] = $database->get('vicidial_campaigns','manual_dial_list_id',['campaign_id'=>$campaign]);
                ...
                $database->insert('custom_fields_data',$arrayInsert);
            }
...
if ( ($ACTION == 'VDADpause') or ($ACTION == 'VDADready') or ($pause_trigger == 'PAUSE') )
	...
	if ( (strlen($vla_status)<2) or (strlen($server_ip)<1) )
		...
		if ( ($ACTION == 'VDADpause') or ($pause_trigger == 'PAUSE') )
			...
			if ( (preg_match("/NULL/i",$dispo_epoch)) or ($dispo_epoch < 1000) )
				{
//				$stmt="UPDATE vicidial_agent_log set sub_status = 'System',wait_sec='$wait_sec' where agent_log_id='$agent_log_id';";
		...
		### this is not the first pause code entry, insert new vicidial_agent_log entry
			...
			if ($VDpr_ct > 0)
				...
				$pause_sec = ($StarTtime - $row[0]);
                        ...
                        $AgentLogStatus = $database->get('vicidial_agent_log','sub_status',['agent_log_id'=>$agent_log_id]);
                        if(empty($AgentLogStatus)){
                          $AgentLogStatus1 = $database->get('vicidial_agent_log','*',['agent_log_id'=>$agent_log_id]);
//                          $AgentLogStatus2 = $database->get('vicidial_agent_log','*',['AND'=>['campaign_id'=>$AgentLogStatus1['campaign_id'],'user'=>$AgentLogStatus1['user']]]);
                          $AgentLogStatus2 = $database->get('vicidial_agent_log','*',['event_time'=>$AgentLogStatus1['event_time']]);
                          $status1 = $AgentLogStatus2['sub_status'];
                          $stmt="UPDATE vicidial_agent_log set pause_sec='$pause_sec',sub_status='".@$status1."' where agent_log_id='$agent_log_id';";
...
//                SELECT * from WHERE pause_code
                $PauseCodeTimeLimit = $database->get('vicidial_pause_codes',['time_limit'],['AND'=>['pause_code'=>$status,'campaign_id'=>$_POST['campaign']]]);

	echo _QXZ(" Pause Code %1s has been recorded",0,'',$status)."\nNext agent_log_id:\n" . $agent_log_id . "\nPauseCodeTimeLimit:\n".@$PauseCodeTimeLimit['time_limit']."\n";
...
//	echo "<TABLE CELLPADDING=0 CELLSPACING=1>";
	...
	while ($agents_count > $loop_count)
		...
		if ($status=='READY')
			{
				if($call_time <= 30){
						$statuscolor = 'call-orange';
				}elseif($call_time <= 60){
						$statuscolor = 'call-orange1';
				}elseif($call_time <= 300){
						$statuscolor = 'call-orange2';
				}else{
						$statuscolor = 'call-orange3';
				}
			}
			if($status=='DIAL' or $status=='CLOSER') {
				$statuscolor='call-orange';
			}
		if ($status=='INCALL')
			{
			$statuscolor='call-green';
			}
			if(($status=='DISPO') or ($status=='DEAD')) {
				$statuscolor='call-red';
			}
		if (($status=='PAUSED') or ($status=='QUEUE'))
			{
			$statuscolor='call-yelllow';
			}
		...
		if ($comments=='AgentXferViewSelect')
			...
			{
			echo "<TR class=\"$statuscolor\"><TD><font style=\"font-size: 12px;  font-family: sans-serif;\"> &nbsp; ";
			echo "$row[0] - $row[2] ($status)";
			echo "&nbsp;</font></TD>";
...
//		echo "<TABLE CELLPADDING=0 CELLSPACING=1><TR><TD VALIGN=TOP><TABLE CELLPADDING=0 CELLSPACING=1>";
		...
		while ($j > $k)
			...
			$i = $order_split[1];
                        echo '<option value="'.$AXVSuser[$i].'" class="'.$AXVSstatuscolor[$i].'">'.$AXVSuser[$i].'-'.$AXVSfull_name[$i].' ('.$AXVSstatus[$i].')</option>';
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (preg_match('/NONE/i',$view_calls_in_queue))
		{
//		echo '<p style="width:100%;">'._QXZ("Calls in Queue View Disabled for this campaign")."</p>\n";
...
//		echo "<th>"._QXZ("PHONE")." </th>";
		echo "<th>Phone</th>";
//		echo "<th>"._QXZ("FULL NAME")."</th>";
		echo "<th>Name</th>";
//		echo "<th>"._QXZ("WAIT")."</th>";
		echo "<th>Wait</th>";
//		echo "<th>"._QXZ("AGENT")."</th>";
//		echo "<th><font style=\"font-size:11px;font-family:sans-serif;\"> &nbsp; &nbsp; &nbsp; </font></TD>";
//		echo "<th>"._QXZ("CALL GROUP")." </th>";
		echo "<th>Ingroup</th>";
//		echo "<th>"._QXZ("TYPE")."</th>";
		echo "<th>Type</th>";
		echo "</tr></thead>";
		...
		while ( ($calls_count > $loop_count) and ($view_calls_in_queue > $loop_count) )
			...
			if ( (preg_match('/Y/i',$grab_calls_in_queue)) and ($OQcall_type[$loop_count]=='IN') )
				{
				echo "<tr $Qcolor>";
				echo "<td> <a href=\"#\" onclick=\"callinqueuegrab('$OQauto_call_id[$loop_count]');return false;\" class='btn btn-success'>Take Call</a></td>";
...
//                        echo "<tr>";
//				echo "<td> <a href=\"#\" onclick=\"callinqueuegrab('ddddd');return false;\" class='btn btn-success'>Tale Call</a></td>";
//				echo "<td>01282526693</td>";
//				echo "<td>---</td>";
//				echo "<td>0.07</td>";
////				echo "<TD><font style=\"font-size: 11px; font-family: sans-serif;\"> &nbsp; $agent - $agent_name &nbsp; </font></TD>";
////				echo "<TD bgcolor=\"$group_color\"><font style=\"font-size: 11px; font-family: sans-serif;\"> &nbsp; &nbsp; &nbsp; </font></TD>";
...
if ($ACTION == 'LEADINFOview')
	...
	#		$INFOout .= "<tr bgcolor=white><td ALIGN=right><font class='sb_text'>Entry List ID: &nbsp; </td><td ALIGN=left><font class='sb_text'>$row[28]</td></tr>";
		...
		if ($hide_call_log_info=='N')
			...
			$NOTESout .= "<thead class='bg-success'><tr>";
			$NOTESout .= "<th># </th>";
			$NOTESout .= "<th>"._QXZ("DATE/TIME")." </th>";
			$NOTESout .= "<th>"._QXZ("AGENT")." </th>";
			$NOTESout .= "<th>"._QXZ("LENGTH")." </th>";
			$NOTESout .= "<th>"._QXZ("STATUS")." </th>";
			$NOTESout .= "<th>"._QXZ("PHONE")." </th>";
			$NOTESout .= "<th>"._QXZ("CAMPAIGN")."</th>";
			$NOTESout .= "<th>"._QXZ("IN/OUT")." </th>";
			$NOTESout .= "<th>"._QXZ("ALT")." </th>";
			$NOTESout .= "<th>"._QXZ("HANGUP")." </th>";
		#	$NOTESout .= "</TR><TR>";
		#	$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\" COLSPAN=9><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; FULL NAME &nbsp; </font></TD>";
			$NOTESout .= "</tr></thead><tbody>";
...
//				$NOTESout .= "<tr $bgcolor>";
				$NOTESout .= "<tr>";
				$NOTESout .= "<td>$u</td>";
				$NOTESout .= "<td>$ALLcall_date[$i]</td>";
				$NOTESout .= "<td>$ALLuser[$i]</td>";
				$NOTESout .= "<td>$ALLlength_in_sec[$i]</td>";
				$NOTESout .= "<td>$ALLstatus[$i]</td>";
				$NOTESout .= "<td>$ALLphone_code[$i] $phone_number_display </td>";
				$NOTESout .= "<td>$ALLcampaign_id[$i] </td>";
				$NOTESout .= "<td>$ALLin_out[$i] </td>";
				$NOTESout .= "<td>$ALLalt_dial[$i] </td>";
				$NOTESout .= "<td>$ALLhangup_reason[$i] </td>";
				$NOTESout .= "</TR>";
//				$NOTESout .= "<TR>";
//				$NOTESout .= "<td></td>";
...
//				$NOTESout .= "</tr>\n";
		...
		if ($allow_emails>0)
			...
			$NOTESout .= "<th>"._QXZ("AGENT")." </th>";
			$NOTESout .= "<th>"._QXZ("CAMPAIGN")." </th>>";
			$NOTESout .= "<th>"._QXZ("EMAIL TO")."</th>";
			$NOTESout .= "<th>"._QXZ("ATTACHMENTS")."</th>";
			$NOTESout .= "</tr></thead><tbody>";
...
if ($ACTION == 'CalLBacKLisT')
	...
	if($start_date) {
		$callback_date = "and DATE(callback_time) BETWEEN '$start_date' and '$end_date'";
	} else {
		$callback_date = "and DATE(callback_time) = '$curr_date'";
	}

	$stmt = "SELECT callback_id,lead_id,campaign_id,status,entry_time,callback_time,comments,phone_number from vicidial_callbacks where recipient='USERONLY' and user='$user' $campaignCBsql $campaignCBhoursSQL $campaignCBdisplaydaysSQL $callback_date and status NOT IN('INACTIVE','DEAD') AND `vicidial_callbacks`.`callback_id` IN (
		SELECT MAX(callback_id)
		FROM `vicidial_callbacks`
		group by `vicidial_callbacks`.`lead_id` ) order by callback_time;";
	if ($DB) {echo "$stmt\n";}
	$rslt=mysql_to_mysqli($stmt, $link);
	...
	while ($callbacks_count>$loop_count)
		...
		$comments[$loop_count]		= preg_replace("/\\n/", "%%", $row[6]);
		$phone[$loop_count]		    = $row[7];
		$loop_count++;
		}
	$loop_count=0;
	while ($callbacks_count>$loop_count)
...
if ($ACTION == 'CalLBacKCounT')
	...
	$curr_date = date("Y-m-d");
	$stmt = "SELECT count(*) from vicidial_callbacks where recipient='USERONLY' and DATE(callback_time) = '$curr_date' and user='$user' $campaignCBsql $campaignCBhoursSQL $campaignCBdisplaydaysSQL and status NOT IN('INACTIVE','DEAD') AND (`vicidial_callbacks`.`callback_id` IN (
    SELECT MAX(callback_id)
    FROM `vicidial_callbacks`
	group by `vicidial_callbacks`.`lead_id`));";
	if ($DB) {echo "$stmt\n";}
	$rslt=mysql_to_mysqli($stmt, $link);

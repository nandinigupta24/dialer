...
require_once 'd_files_1/constant.php';
...
if ($relogin == 'YES')
    ...
    require_once 'd_files_1/part_re_login.php';
    exit;
	}

if ($user_login_first == 1)
	{
    require_once 'd_files_1/user_login_first.php';
	exit;
	}

if ( (strlen($phone_login)<2) or (strlen($phone_pass)<2) )
	{
	require_once 'd_files_1/part_login.php';
        exit;
	}
else
	{
	...
	if ( (strlen($VD_login)<2) or (strlen($VD_pass)<2) or (strlen($VD_campaign)<2) )
		...
		if($auth>0){
                    if(!empty($OLDDesign) && $OLDDesign == 1){
                        require_once 'd_files_1/dash_header.php';
                    }else{
                        require_once 'd_files_1/dash_header_new.php';
                    }
                }else
			{
			if ($WeBRooTWritablE > 0)
	...
	if ($VDloginDISPLAY){
            require_once 'd_files_1/part_campaign_login_error.php';
            exit;
		}
	...
	if (!$authphone)
		...
		require_once 'd_files_1/part_phone_login_error.php';
		exit;
		}
	else
		{
        ...
        #   echo "<b>You have logged in as user: $VD_login on phone: $SIP_user to campaign: $VD_campaign</b><br>\n";
			...
			{
                    $VDdisplayMESSAGE = 'Sorry, there are no leads in the hopper for this campaign';
                    require 'd_files_1/part_lead_login_error.php';
...
<?php 
if(!empty($OLDDesign) && $OLDDesign == 1){
require('d_files_1/script.php');
?>
</head>
<?php

$zi=2;
require 'd_files_1/dashboard.php';
?>
...
<?php
    ...
    require_once 'd_files_1/new_design_2.php';
    exit;
    ?>

...
function user_authorization($user, $pass, $user_option, $user_update, $bcrypt, $return_hash, $api_call=NULL, $source=NULL) {
...
function custom_list_fields_values($lead_id, $list_id, $uniqueid, $user, $DB, $call_id, $did_id, $did_extension, $did_pattern, $did_description, $dialed_number, $dialed_label) {
        ...
        if ($fieldscount_to_print > 0) {
            ...
            while ($fields_to_print > $o) {
                ...
                if (($A_field_type[$o] == 'SELECT') or ( $A_field_type[$o] == 'MULTI') or ( $A_field_type[$o] == 'RADIO') or ( $A_field_type[$o] == 'CHECKBOX') or ( $A_field_type[$o] == 'SWITCH')) {
                    ...
                    $te_printed = 0;
                    if ($DB > 0) {
                        echo "DEBUG: |$A_field_id[$o]|$A_field_label[$o]|$A_field_name[$o]|$A_field_type[$o]|$A_field_options[$o]|$field_options_count|\n";
                    }
                        ...
                        if (preg_match("/,/", $field_options_array[$te])) {
                            ...
                            if (($A_field_type[$o] == 'SELECT') or ( $A_field_type[$o] == 'MULTI')) {
                                if (strlen($A_field_value[$o]) > 0) {
                                    ...
                                    if ($DB > 0) {
                                        echo "DEBUG2: |$field_options_value_array[0]|$A_field_value[$o]|$field_selected|\n";
                                    }
                                } else {
                            ...
                            if (($A_field_type[$o] == 'RADIO') or ( $A_field_type[$o] == 'CHECKBOX')) {
                                ...
                                if (strlen($A_field_value[$o]) > 0) {
                                    ...
                                    if ($DB > 0) {
                                        echo "DEBUG3: |$temp_options_value|$A_field_value[$o]|$field_selected|\n";
                                    }
                                } else {
                                    if ($A_field_default[$o] == "$field_options_value_array[0]") {
                                        $field_selected = 'CHECKED';
                ...
                if ($A_field_type[$o] == 'TEXT') {
                    ...
                    if (($duplicates_count > 0) and ( (preg_match("/\|$A_field_label[$o]\|/i", $duplicates_master_list)) or ( preg_match("/\|$A_field_label[$o]\|/i", $duplicates_list)) )) {
                        ...
                        if (preg_match("/\|$A_field_label[$o]\|/i", $duplicates_master_list)) {
                            ...
                            while ($dup_fields_ct > $df) {
                                if (preg_match("/$master_field_match/", $dup_fields[$df])) {
                                    $update_dup_fields .= "$dup_fields[$df]|";
                                    $update_dup_ct++;
                                }
                                $df++;
                            }
                        ...
                        if (preg_match("/\|$A_field_label[$o]\|/i", $duplicates_list)) {
                            ...
                            $update_dup_fields .= "$master_field|";
                            $update_dup_ct++;
                            $master_field_match = $master_field . '_DUPLICATE_.*';
                            $dup_fields = explode('|', $duplicates_list);
                            $dup_fields_ct = count($dup_fields);
    ...
    if (preg_match("/--A--|--U--/", $CFoutput)) {
        ...
        if (preg_match('/--A--list_|--U--list_/i', $CFoutput)) {
            ...
            $rslt = mysql_to_mysqli($stmt, $link);
            if (isset($mel) && $mel > 0) {
                mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '05021', $user, $server_ip, $session_name, $one_mysql_log);
            }
        ...
        if (preg_match('/--A--did_|--U--did_/i', $CFoutput)) {
            ...
            $rslt = mysql_to_mysqli($stmt, $link);
            if (isset($mel) && $mel > 0) {
                mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '05022', $user, $server_ip, $session_name, $one_mysql_log);
            }
...
function custom_list_fields_values_NEW($lead_id, $list_id, $uniqueid, $user, $DB, $call_id, $did_id, $did_extension, $did_pattern, $did_description, $dialed_number, $dialed_label) {
    $STARTtime = date("U");
    $TODAY = date("Y-m-d");
    $NOW_TIME = date("Y-m-d H:i:s");
...
function get_stats_log($database,$DBUTG,$user,$campaign,$event,$other){
    $UserDetails = $database->get('vicidial_users',['user_group','experience'],['user'=>$user]);
    switch($event){
        case 'on_sale':
            $points = $DBUTG->get('agent_events','points',['AND'=>['event'=>'on_sale','user_group'=>$UserDetails['user_group']]]);
            if(!empty($points)){
                ...
                if(!empty($ExistCount) && $ExistCount > 0 ){
                    $DBUTG->insert('agent_logs',['event'=>'on_sale','user'=>$user,'points'=>$points,'created_at'=>date('Y-m-d H:i:s'),'experience'=>$UserDetails['experience']]);
                }
            }
            break;
        case 'on_time_login':
            $ExistCount = $DBUTG->count('agentshifts',['AND'=>['campaign'=>$campaign,'user'=>$user,'user_group'=>$UserDetails['user_group']]]);
            if(!empty($ExistCount) && $ExistCount > 0 ){
                ...
                if($StartTime >= $other){
                    ...
                    if(!empty($ExistLog) && $ExistLog > 0){
                        ...
                        $DBUTG->insert('agent_logs',['event'=>'on_login_time','user'=>$user,'points'=>$points,'created_at'=>date('Y-m-d H:i:s'),'experience'=>$UserDetails['experience']]);
                    }
                }
            }
            break;
...
function get_expired_check($UserGroup,$database,$DBUTG){
    if($UserGroup != 'ADMIN'){
        $exist = $DBUTG->count('user_pricings',['AND'=>['status'=>'RUN','user_group'=>$UserGroup]]);
        if($exist == 0){
            header('location:../utgone/welcome?plan=expired');
            exit;
        }
    }
}
...
function get_plan_agent_login($UserGroup,$database,$DBUTG){
    if($UserGroup != 'ADMIN'){
         if($UserGroup != 'ADMIN'){
        $exist = $DBUTG->get('user_pricings','*',['AND'=>['status'=>'RUN','user_group'=>$UserGroup]]);
        $PlanAgents = $DBUTG->get('pricing_plans','agent',['id'=>$exist['pricing_plan_id']]);
...
ON
VLA.user = VU.user WHERE VU.user_group = '".$UserGroup."'";

        $LoginAgents = $database->query($query)->fetchAll(PDO::FETCH_ASSOC);

        if($LoginAgents[0]['total'] >= $PlanAgents){
            header('location:../utgone/welcome?plan=more_agent');
            exit;
        }
    }
    }

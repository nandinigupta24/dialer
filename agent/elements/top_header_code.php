...
//echo strrpos('ALL-CAMPAIGNS',$VD_campaign);
//echo
//exit;
//if ((!preg_match("/\s$VD_campaign\s/i", $LOGallowed_campaigns)) and ( !preg_match("/ALL-CAMPAIGNS/i", $LOGallowed_campaigns)))
////if (!is_numeric(strrpos($LOGallowed_campaigns, $VD_campaign)) && !is_numeric(strrpos('ALL-CAMPAIGNS', $VD_campaign)) )
//        {
...
//exit;
    ...
    $HideAgentCallLogs = $row[135];
    $HideCustomerCallHistory = $row[136];
    $DTMFKeypad = $row[137];
    $call_tags = $row[138];
    $call_timer = $row[139];
    $campaign_name = $row[140];
    $inbound_call_drop = $row[141];
    $hide_phone = $row[142];
    $inbound_call_popup = $row[143];
    ...
    if (isset($queuemetrics_pe_phone_append) and ($queuemetrics_pe_phone_append > 0) and ( strlen($qm_phone_environment) > 0)) {
...
//    if(!empty($VU_closer_campaigns)){
//                                           $a1 = explode(' ',$VU_closer_campaigns);
//                                           $a2 = explode(' ',$closer_campaigns);
//                                           $result=array_intersect($a1,$a2);
//                                           $closer_campaigns = implode(' ',$result);
//                                        }
    ...
    if ((preg_match('/Y/', $agent_pause_codes_active)) or ( preg_match('/FORCE/', $agent_pause_codes_active))) {
        ...
        while ($j < $VD_pause_codes)
					...
					//$VD_pause_codes_ct = isset($VD_pause_codes_ct) ? ($VD_pause_codes_ct+$VD_pause_codes) : $VD_pause_codes;
    ...
    if ($allow_closers == 'Y') {
        ...
        while ($XFgrpCT < $xfer_ct) {
            ...
            $VARxfergroupsnames = isset($VARxfergroupsnames) ? "$VARxfergroupsnames'$row[1]'," : '';
        ...
        $VARxfergroupsnames = isset($VARxfergroupsnames) ? substr("$VARxfergroupsnames", 0, -1) : '';
    }
    ...
    echo "<!-- $campaign_leads_to_call - leads left to call in hopper123 -->\n";
} else {
    $VDloginDISPLAY = 1;

...
require_once 'd_files/constant.php';
...
if ($relogin == 'YES')
    ...
    require_once 'd_files/part_re_login.php';
    exit;
	}

if ($user_login_first == 1)
	{
    require_once 'd_files/user_login_first.php';
	exit;
	}

if ( (strlen($phone_login)<2) or (strlen($phone_pass)<2) )
	{
	require_once 'd_files/part_login.php';
        exit;
	}
else
	{
	...
	if ( (strlen($VD_login)<2) or (strlen($VD_pass)<2) or (strlen($VD_campaign)<2) )
		...
		if($auth>0){
                    if(!empty($OLDDesign) && $OLDDesign == 1){
                        require_once 'd_files/dash_header.php';
                    }else{
                        require_once 'd_files/dash_header_new.php';
                    }
                }else
			{
			if ($WeBRooTWritablE > 0)
	...
	if ($VDloginDISPLAY){
            require_once 'd_files/part_campaign_login_error.php';
            exit;
		}
	...
	if (!$authphone)
		...
		require_once 'd_files/part_phone_login_error.php';
		exit;
		}
	else
		{
...
<?php 
if(!empty($OLDDesign) && $OLDDesign == 1){
require('d_files/script.php');
?>
</head>
<?php

$zi=2;
require 'd_files/dashboard.php';
?>
...
<?php
    ...
    require_once 'd_files/new_design.php';
    exit;
    ?>
